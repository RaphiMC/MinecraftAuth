 ...
 */
package net.raphimc.minecraftauth.java.responsehandler;
...
public interface MinecraftServicesResponseHandler<R> extends JsonHttpResponseHandler<R> {
    ...
    default void handleError(final HttpResponse response, final GsonObject json) throws IOException {
        if (json.hasString("error") && json.hasString("errorMessage")) {
            throw new MinecraftServicesRequestException(response, json.reqString("error"), json.reqString("errorMessage"));
        } else if (json.hasString("errorMessage")) {
            throw new InformativeHttpRequestException(response, json.reqString("errorMessage"));
        }
    }
