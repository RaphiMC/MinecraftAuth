...
public interface MinecraftResponseHandler<R> extends JsonHttpResponseHandler<R> {
    ...
    default void handleError(final HttpResponse response, final GsonObject json) throws IOException {
        if (json.hasString("error") && json.hasString("errorMessage")) {
            throw new MinecraftRequestException(response, json.reqString("error"), json.reqString("errorMessage"));
